...
function renderAnswer(text, accent, dark, idPrefix) {
  return text.split(/(\[\d+\])/g).map((part, i) => {
    ...
    return m
      ? <Cite key={i} num={+m[1]} accent={accent} dark={dark} idPrefix={idPrefix} />
...
export default function App() {
  ...
  const resetChat = () => {
    ...
    setQuery_(""); setMessages([]); setLoading(false); setPendingQuestion("");
  ...
  const submit = useCallback((q) => {
    ...
    setQuery_(""); setLoading(true); setPendingQuestion(question);

    if (isNewConversationRef.current) {
      setHistory(h => [question, ...h.filter(x => x !== question)].slice(0, 12));
      isNewConversationRef.current = false;
    }

    query(question).then(data => {
      if (generationRef.current !== gen) return;
      setLoading(false);
      setPendingQuestion("");
      setMessages(prev => [...prev, { question, answer: "", sources: data.sources, followUps: data.followUps, done: false }]);
      let i = 0;
      const text = data.answer;
      streamRef.current = setInterval(() => {
        if (generationRef.current !== gen) { clearInterval(streamRef.current); return; }
        i += Math.floor(Math.random() * 4) + 2;
        if (i >= text.length) {
          setMessages(prev => { const m = [...prev]; m[m.length - 1] = { ...m[m.length - 1], answer: text, done: true }; return m; });
          clearInterval(streamRef.current);
          inputRef.current?.focus();
          return;
        }
        setMessages(prev => { const m = [...prev]; m[m.length - 1] = { ...m[m.length - 1], answer: text.slice(0, i) }; return m; });
      }, 26);
    });
  }, [query_]);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: th.bg, transition: "background 0.2s,color 0.2s" }}>
      ...
      <div style={{ flex: 1, display: "flex", maxWidth, margin: "0 auto", width: "100%", padding: `0 ${px}` }}>
        ...
        {/* History sidebar */}
        {showSidebar && (
          <aside className="slide-in" style={{ width: 200, flexShrink: 0, borderRight: `1px solid ${th.border}`, paddingTop: 24, paddingRight: 16, paddingBottom: 24, transition: "border-color 0.2s" }}>
            ...
            {history.length === 0
              ? <div style={{ fontSize: 12, color: th.textFaint, fontFamily: SANS, lineHeight: 1.6 }}>Noch keine Suchanfragen.</div>
              : history.map((h, i) => (
                <button key={i} onClick={() => { resetChat(); submit(h); }} style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: "none", borderBottom: `1px solid ${th.border}`, cursor: "pointer", fontSize: 12, color: th.textMid, padding: "9px 0", lineHeight: 1.5, transition: "color 0.1s", fontFamily: serif }}
      ...
      <div style={{ position: "sticky", bottom: 0, background: `linear-gradient(transparent,${th.bg} 32%)`, padding: `16px ${px} 22px`, transition: "background 0.2s" }}>
        <div style={{ maxWidth, margin: "0 auto" }}>
          <div style={{ paddingLeft: showSidebar ? 224 : 0 }}>
            <div
