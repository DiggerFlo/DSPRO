...
def precision_at_k(retrieved_ids: list[str], relevant_ids: set[str], k: int) -> float:
    hits = sum(1 for id_ in retrieved_ids[:k] if id_ in relevant_ids)
...
def mrr(retrieved_ids: list[str], relevant_ids: set[str]) -> float:
    for i, id_ in enumerate(retrieved_ids, start=1):
...
def evaluate_retrieval(results: list[dict], relevant_ids: list[str], k: int = 5) -> dict:
    """Compute Precision@k, Recall@k, MRR and NDCG@k for one query."""
